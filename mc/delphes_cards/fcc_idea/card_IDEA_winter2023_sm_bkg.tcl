####################################################################
# FCC-ee IDEA detector model -- SM BACKGROUND VALIDATION CARD
#
# Base card: card_IDEA_winter2023.tcl
# Authors (original): Elisa Fontanesi, Lorenzo Pezzotti, Massimiliano Antonello, Michele Selvaggi
# email: efontane@bo.infn.it, lorenzo.pezzotti01@universitadipavia.it,
#        m.antonello@uninsubria.it, michele.selvaggi@cern.ch
#
# *** SM INCLUSIVE BACKGROUND VALIDATION CARD -- NOT a validated FCC-ee physics result ***
#
# Modifications vs card_IDEA_winter2023.tcl (each marked "SMBKG:" below):
#   - GenJetFinder and FastJetFinder: JetAlgorithm 10 (Durham exclusive kt, ee-specific)
#     replaced with JetAlgorithm 6 (anti-kT, R=0.5).
#
# Why: the Durham kt algorithm in FastJet throws a fastjet::Error -> SIGSEGV when
# its input particle list is empty on a per-event basis. In a mixed SM sample ~5%
# of events are e+e- -> nu nu~ (purely invisible). After NeutrinoFilter removes the
# neutrinos, GenJetFinder receives zero particles for those events; FastJetFinder
# likewise receives an empty EFlowMerger for invisible events. Durham kt crashes on
# each such event. Anti-kT returns an empty jet collection for zero-particle input
# without crashing, making it safe for mixed hadronic+invisible samples.
#
# Physics note: Durham kt is the theoretically preferred algorithm for exclusive jet
# counting in the e+e- spherical geometry (the JADE/LUCLUS lineage). Anti-kT with
# R=0.5 is more LHC-like (inclusive jets, pT-ordered). For this test the priority
# is getting a crash-free full-object ROOT file; production cards should revisit
# the jet algorithm choice.
#
# All other modules (tracking, calorimeter, photon, electron, muon, b-tagging,
# tau-tagging, MissingET, TreeWriter) are IDENTICAL to card_IDEA_winter2023.tcl.
####################################################################
...
# SMBKG: JetAlgorithm 10 (Durham kt) -> 6 (anti-kT), ParameterR 1.5 -> 0.5, ParameterP removed.
# Durham kt throws fastjet::Error -> SIGSEGV when input is empty (nu nu~ events).
# Anti-kT returns an empty jet collection for zero-particle input without crashing.
...
# SMBKG: JetAlgorithm 10 (Durham kt) -> 6 (anti-kT), ParameterR 1.5 -> 0.5, ParameterP removed.
# Same rationale as GenJetFinder: EFlowMerger is empty for nu nu~ events.
