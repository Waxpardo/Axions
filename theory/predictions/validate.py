...
def gate1_cross_section(banner_path: Path, m_a: float, g_agg: float, sqrt_s: float, tol: float = 0.05) -> dict[str, Any]:
    ...
    theory_pb = float(sigma_prod_pb(m_a, g_agg, sqrt_s))
...
def gate2_width(width_gev_from_mg: float, m_a: float, g_agg: float, tol: float = 0.05) -> dict[str, Any]:
    theory_width = float(gamma_a(m_a, g_agg))
...
def validate_recoil_photon(lhe_path: Path, m_a: float, sqrt_s: float, plot_path: Path | None = None) -> dict[str, Any]:
    ...
    expected = float(e_gamma_recoil(m_a, sqrt_s))
    energies: list[float] = []
...
def validate_angular(lhe_path: Path, m_a: float, sqrt_s: float, plot_path: Path | None = None) -> dict[str, Any]:
    pylhe = _require_pylhe()
    expected_energy = float(e_gamma_recoil(m_a, sqrt_s))
...
def validate_decay_length(
    ...
    theory = float(ell_a(m_a, g_agg, sqrt_s))
    mc_mean = float(values.mean())
...
def validate_opening_angle(
    ...
    with pyhepmc.open(str(hepmc_path)) as handle:
        for event in handle:
            for particle in event.particles:
                ...
                v1 = np.array([p1.px, p1.py, p1.pz], dtype=float)
                v2 = np.array([p2.px, p2.py, p2.pz], dtype=float)
                denom = np.linalg.norm(v1) * np.linalg.norm(v2)
                if denom == 0:
                    continue
                cos_angle = float(np.dot(v1, v2) / denom)
                opening_angles.append(float(np.arccos(np.clip(cos_angle, -1.0, 1.0))))
    if not opening_angles:
        raise ValueError(f"No gamma gamma ALP decays with PDG {alp_pdg_id} found in {hepmc_path}")

    values = np.asarray(opening_angles)
    theory = float(delta_theta_min(m_a, sqrt_s))
    mc_min = float(values.min())

    if plot_path:
        plt = _require_pyplot()
        plt.figure()
        plt.hist(np.degrees(values), bins=50, alpha=0.75, label="MC")
        plt.axvline(np.degrees(theory), color="red", linestyle="--", linewidth=2, label=f"Theory {np.degrees(theory):.3g} deg")
    ...
    return {
        ...
        "mc_min_deg": float(np.degrees(mc_min)),
        "theory_min_deg": float(np.degrees(theory)),
        "n_decays": int(values.size),
    }
